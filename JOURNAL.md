...
## 2026-05-04 — Session 017: Soft launch — staging the moment

Phase 3 begins. Called an audible on the roadmap (which had soft launch slotted as session 019, after a glossary and template+gallery) to launch now. Locked in the channel set: Show HN, Bluesky, LinkedIn, r/internetisbeautiful with a crosspost to r/coolwebsites. Spot-checked the live site — animations, OG card, corner link, analytics all healthy — and confirmed the only cosmetic note (opengraph.xyz wants a longer OG title) isn't blocking. Picked **Wed May 6, 8:30 AM ET** as the headline slot for HN, with Bluesky + LinkedIn firing 30 min later to amplify the first hour, and Reddit deferred to Saturday May 9 morning so the project gets a second peak instead of burning all four channels in one day. Drafted all five posts as copy-paste blocks (Show HN with seeded OP first comment, IIB with seeded comment, r/coolwebsites crosspost, Bluesky four-skeet thread with per-skeet character counts, LinkedIn long-form), opened HN signup for Steve to register `SteveRyherd`, and wrote a nine-question response playbook for predictable launch-day questions ("did you really not write code?", "why CC BY-NC?", "what is COPPA?", etc.). No site code changed in this session — all infrastructure for what comes next.

→ Full transcript: [sessions/017-soft-launch.md](./sessions/017-soft-launch.md)
